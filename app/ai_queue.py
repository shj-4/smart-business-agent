"""
طابور معالجة غير متزامن خلف تحليل الـ AI (Gemini) — اختياري.

عند تفعيله (AI_QUEUE_ENABLED=true) تُنفَّذ مهام التعرف/التحليل بحدّ توافق
(concurrency) وبسعة طابور محدودة (maxsize) بدل إطلاق كل طلب مباشرةً على Gemini
دفعة واحدة وقت الذروة. فوق سعة الطابور تنشأ "ضغط رجعي" (backpressure): المتصل
ينتظر حتى يتسع مساحة — الطريقة لا تُسقط رسائل ولا تُكرّرها.

الوحدة المعالَجة عمل متزامن (function تُنفَّذ في خيط عبر asyncio.to_thread) كي
تظل حلقة الأحداث مستجيبة حتى مع استدعاءات Gemini المتزامنة الحالية.

عند إيقافه (الافتراضي) يُنفَّذ العمل مباشرةً — نفس سلوك اليوم تمامًا.
"""
