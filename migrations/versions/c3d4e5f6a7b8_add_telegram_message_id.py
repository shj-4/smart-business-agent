...
down_revision: str | Sequence[str] | None = "29ef2f3a7f5d"
...
def upgrade() -> None:
    ...
    if "notes" not in insp.get_table_names():
        ...
        op.create_index(
            "ix_notes_telegram_message_id", "notes", ["telegram_message_id"], unique=True
        )
    ...
    if "telegram_message_id" not in trans_cols:
        ...
        op.create_index(
            "ix_transactions_telegram_message_id",
            "transactions",
            ["telegram_message_id"],
            unique=True,
        )
    ...
    if "telegram_message_id" not in task_cols:
        with op.batch_alter_table("tasks") as batch_op:
            batch_op.add_column(sa.Column("telegram_message_id", sa.BigInteger(), nullable=True))
        op.create_index(
            "ix_tasks_telegram_message_id", "tasks", ["telegram_message_id"], unique=True
        )
